...
- [x] Task 2.6: Create wgpu Surface from GLFW window native handle
...
- [x] Task 3.5: Implement `RenderFrame`: acquire surface texture → begin
      ...
      submit → present
- [x] Task 3.6: Wire `GPUBackend.RenderFrame` in `pkg/renderer/gpu.go` to
...
- [x] Task 4.1: Update `pkg/renderer/factory.go` — `--backend auto` tries
      `GPUBackend.Init()` first; falls back to `CPUBackend` on any error
- [x] Task 4.2: Confirm `go test -tags=headless ./...` still passes (`pkg/gpu`
      needs no headless tag; CPU path unchanged)
- [x] Task 4.3: Audit `.github/workflows/ci.yml` — verify `pkg/gpu` is covered
...
- [x] Task 4.4: Full quality gates: `go fmt ./...`, `go vet ./...`,
      `golangci-lint run --build-tags headless`, `go test -tags=headless
      -cover ./...`
- [x] Task 4.5: Update `conductor/architecture.md` — add `pkg/gpu` API
      summary table, coverage, and Zero-CGo note
- [x] Task 4.6: Update `conductor/tracks.md` status and `CLAUDE.md` Recent
      Updates

## Final Verification

- [x] All 5 acceptance criteria met
- [x] `go test -tags=headless ./...` passes (race detector skipped on Windows)
- [x] `golangci-lint run --build-tags headless` passes (0 issues)
- [ ] CI green on all matrix entries (pending PR push)
- [x] Docs updated: architecture.md, tracks.md, CLAUDE.md
- [x] `.github/workflows/ci.yml` reviewed and updated (Go 1.24→1.25; pkg/gpu assets comment added)
